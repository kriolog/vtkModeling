...
int vtkMovingLeastSquare::RequestData(
    ...
    for (int i=0; i<this->Ids->GetNumberOfIds(); i++) {
        this->P->InsertNextPoint(pts->GetPoint(this->Ids->GetId(i)));
    }
    ...
    for (int i=0; i<this->pts->GetNumberOfPoints(); i++) {
        if (this->Ids->IsId(i) != -1) {
            (*ptMat)(i,0)=(*QMat)(this->Ids->IsId(i), 0);
            (*ptMat)(i,1)=(*QMat)(this->Ids->IsId(i), 1);
            if (this->Dimension == 3) {
                (*ptMat)(i,2)=(*QMat)(this->Ids->IsId(i), 2);
            }
            continue;
        }
